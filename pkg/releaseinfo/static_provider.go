...
func (p *StaticProviderDecorator) Lookup(ctx context.Context, image, pullSecretName string) (*ReleaseImage, error) {
	p.lock.Lock()
	defer p.lock.Unlock()

	releaseImage, err := p.Delegate.Lookup(ctx, image, pullSecretName)
	if err != nil {
		return nil, err
	}
